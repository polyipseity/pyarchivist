...
- Prefer `anyio.Path` for file/script identifiers and async filesystem operations. APIs that accept file/script identifiers must accept `os.PathLike` or `anyio.Path` (for example, call sites may pass `Path(__file__)` where `Path` is `anyio.Path`). In tests prefer annotating the `pytest` `tmp_path` fixture as `tmp_path: os.PathLike[str]` (do not use `pathlib.Path` or plain `Path` annotations). When a string path is required, always use `os.fspath(path_like)` rather than `str(path_like)`.
- Do not import or call `asyncio` primitives; the project migrates to AnyIO and Asyncer. Use Asyncer helpers (`from asyncer import create_task_group, soonify, asyncify, runnify, syncify`) for structured concurrency instead of `asyncio.gather` or `asyncio.create_task`. Runnify is used for CLI entrypoints and syncify can expose async APIs to otherwise synchronous callers.
...
- Pydantic models (v2): prefer `model_config = ConfigDict(...)` for model configuration. Import `ConfigDict` from `pydantic` and avoid legacy inner `Config` classes or plain dict literals. Example: `model_config = ConfigDict(frozen=True, populate_by_name=True)` — this is the canonical pydantic v2 pattern for model configuration.
- When creating new Python package directories (source under `src/` or mirrored test packages under `tests/`), always add an `__init__.py` file so the directory is an explicit Python package. This rule applies to both production code and test code — add `__init__.py` for any new package-like subfolder and update the corresponding `tests/` tree.
