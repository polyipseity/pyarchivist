...
- Use pre-commit-style hooks (for example via `prek`, using hooks such as `detect-aws-credentials` and `detect-private-key`) to catch accidental commits of credentials prior to pushing.