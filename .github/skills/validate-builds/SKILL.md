...
3. Inspect wheel contents
   ...
   - Check that the top-level module is present (e.g., `pyarchivist/` files) and that small data files are in the module root or `.data` directory as expected.
...
7. Optional deeper checks
   ...
   - Use `pip show -f pyarchivist` to list installed files and confirm layout.
